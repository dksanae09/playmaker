...
export default function ChatView() {
  ...
  const getMessages = useQuery(api.messages.get, {
  ...
  function sendMessage() {
    ...
    try {
      addMessage({
        ...
        fromUser: userId,
        toUser:
          userId === playground?.owner ? playground.editor : playground?.owner,
      });
      setInput("");
  ...
  return (
    <Card>
      <CardContent className="py-2">
        <ScrollArea
          ref={chatRef}
          className="absolute flex h-[400px] flex-col rounded-md p-4"
        >
          {getMessages?.map((message) => {
            return message.fromUser === userId ? (
              <CardContent className="flex flex-row justify-end">
                {decryptText(message.message)}
              </CardContent>
            ) : (
              <CardContent className="m-0 flex w-fit flex-row justify-end rounded-md bg-accent p-2">
                {decryptText(message.message)}
              </CardContent>
            );
          })}
